...
public interface GameRepository extends JpaRepository<Game, Long> {
    ...
    @Query(value = "SELECT * FROM game WHERE (game.date_match BETWEEN ?1 AND ?2) AND game.season_id = ?3", nativeQuery = true)
    List<Game> listMatchesByDateInterval(LocalDate date1, LocalDate date2, Long seasonId);
}
